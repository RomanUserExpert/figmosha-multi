...
> **Обновление 2026-09-24, после замеров (§5).** Гипотеза F5 / вариант B1 («гидратацию
> вызывает чтение `.children` у инстанса») **опровергнута**: обход, который детей
> инстансов не трогает, стоит те же 3,5–5 с. **B2 (`skipInvisibleInstanceChildren`)
> подтверждён**: холодная секция 3,1–3,7 с → 0,2–0,7 с. Память копят **загрузки страниц
> (сотни МБ) и новые ключи компонентов (десятки МБ)**, а не секции (единицы МБ). Шаг 1
> и B2 сделаны в 3.1.0; бюджет вкладки (шаг 3) надо считать в страницах и ключах.
...
## 5. Результаты замеров 2026-09-24

Файл «UIR - FM - Controls», Figmosha 3.0.1 (копия `E:\Work\Compatibl\Figmosha`),
вкладка открыта заново (~1 ГБ приватной памяти процесса на старте, текущая страница —
Table control). Память — **приватные байты процесса вкладки Figma** (Windows
`Get-Process`, pid вкладки определён по приросту), не метр Figma; снимок через 3 с после
каждого `exec`. Секции — уровень-2 юниты страниц 10 и 2, в этой вкладке ещё не тронутые.
Сырые данные — `scratchpad/m/results.jsonl` сессии (вне репозитория).

| Замер | n | Время | Δ памяти вкладки |
|---|---|---|---|
| Загрузка страницы 10 (`getNodeByIdAsync` + `loadAsync`) | 1 | 5,7 с | **+638 МБ** |
| (a) обход без чтения `.children` у INSTANCE, холодная секция | 3 | 3,5–5,1 с | +15…+32 МБ |
| (b) текущий `h.walk` на тех же секциях после (a) | 3 | 2–5 мс | ~0 |
| (b) текущий `h.walk`, холодная секция | 13 | 3,1–3,7 с | +1…+17 МБ, один раз +96 |
| (b) + `skipInvisibleInstanceChildren = true`, холодная секция | 10 | **0,19–0,73 с** | +0…+9 МБ |
| (c) первый `h.mainOf` на 6 новых ключей | 1 | 6,2 с | **+264 МБ** |
| (c) `h.mainOf` на уже загруженные ключи | 5 | 0–2 мс | 0 |
| `setCurrentPageAsync(COVER)` с тяжёлой страницы, +45 с | 1 | 0,5 с | 1945 → 1975 МБ (не освободилось) |

За ~40 холодных секций вкладка выросла суммарно примерно на 130 МБ (без учёта
`mainOf`); к концу — ~2,06 ГБ приватных, жива.

**Выводы.**

1. **B1 не работает.** Стоимость — в материализации самой ноды инстанса при чтении
   `.children` его родителя; читать ли потом детей инстанса — неважно. Правка `:583`
   не нужна.
2. **B2 работает, и сильно — в 10–15 раз по времени.** Сборка скрытых слоёв (строк,
   состояний таблиц) внутри инстансов и была «холодной» стоимостью. На результат скана
   с обрезкой на INSTANCE не влияет. Сделано в 3.1.0 как дефолт `h.walk` (`skipInvisible:
   false` — выключить, `find --hidden`).
3. **Память копят страницы и ключи компонентов.** Секция — единицы МБ, тяжёлая страница —
   сотни, новый библиотечный компонент — десятки. Бюджет вкладки (шаг 3) считать в
   **загруженных тяжёлых страницах и новых ключах `mainOf`**, не в юнитах или секундах.
   Порядок обхода (F): одна тяжёлая страница на сессию вкладки.
4. **Ничего не освобождается** (подтверждение F1/F3): переключение страницы не вернуло
   память.
5. Не проверено: метр Figma (View → Memory usage) против приватных байтов; M1 (закрытие
   плагина), M4 (REST), M6 (переоткрытие) — не делались.

**Сделано в 3.1.0** (ветка `memory-ceiling-plan`): B2 дефолтом; шаг 1 — `each` не
считает `partial` готовым, пишет `started`, `--resume` по последней записи на id и не
перезапускает уже раздробленный юнит; `sessions` показывает `STALLED`. Следующее —
шаг 3 с пересчётом бюджета по п. 3.

**Сделано в 3.2.0.** Шаг 3 — не эвристикой в страницах и ключах, а прямым замером:
`each` перед каждым юнитом (и перед дроблением) читает приватные байты процесса
вкладки (Windows, `K32GetProcessMemoryInfo`; процесс — самый тяжёлый `renderer`
Figma), за `--tab-limit` (1700 МБ) пишет `paused`, ждёт новую сессию плагина
(`--reopen-wait`) и продолжает; иначе код выхода 5. Живая проверка: вкладка на
1974 МБ — ноль запросов в Figma, выход 5. Плюс найденное в том же прогоне: `each`
больше не дробит инстансы (раздробленный `custom-control` не попадал в счёт) и не
выдаёт скрытые слои инстансов юнитами. Дальше — шаг 4 (REST): единственное, что
убирает вкладку из read-only скана совсем.
